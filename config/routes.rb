...
Rails.application.routes.draw do
  ...
  scope '(:locale)', locale: /#{I18n.available_locales.join("|")}/ do #  /#{I18n.available_locales.join("|")}/ - регулярное выражение для передачи допустимых локализаций из application.rb # Теперь мы можем делать марштруты localhost/:locale/topic () говорят что это необязательно, те можно и без них
    ...
    # Один вызов resources может объявить все необходимые маршруты для ваших действий index, show, new, edit, create, update и destroy. # Не менее великолепно, что resources САМ определяет тип необходимых запросов POST,GET а так же (PATCH, PUT, DELETE), но это уже совсем другая история
