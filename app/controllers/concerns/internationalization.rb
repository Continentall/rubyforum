...
module Internationalization
  ...
  included do
    ...
    # &action - любое действие которое контроллеру надо выполнить
    def switch_locale(&)
      locale = locale_from_url || locale_from_headers || I18n.default_locale
      response.set_header 'Content-Language', locale
      I18n.with_locale(locale, &) # действие контроллера будет выполнятся в контексте запрошенной локали
    end
    ...
    # Запутаный метод с github который парсит предпочтиьаемую пользователем локаль
    ...
    # Переопределение встроенного метода Ruby, в котором сказанно к каждому url пристыковываем локаль
